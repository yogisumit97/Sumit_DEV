class Solution {
    public int sumSubarrayMins(int[] arr) {
        int len = arr.length;
        int[] pseIndex = findPseeHelper(arr, len);// stores previous smaller or equal element index
        int[] nseIndex = findNseHelper(arr, len);// stores next smaller index, not equal to avoid double counting of similar elements
        long sum = 0;
        for(int i =0; i<len ; i++){
